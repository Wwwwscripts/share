...
{	
	...
	/*==== title bar ====*/				
		...
		titleBar.rows[0].insertCell(-1);
		titleBar.rows[0].cells[0].innerHTML = '<b>JmÃ©no vesnice</b> (max 32 znakÅ¯) - Fixed by LilGhost';
	...
	if(useDefaultConfig)
	{
		...
		var options = [
			...
			{
				name:'sector',
				description:'kontinent:sektor:pole <img id="twcheese_sector_help" src="http://cdn.tribalwars.net/graphic/questionmark.png" style="width: 13px; height: 13px" title="Mapa je rozdÄ›lena zleva doprava, shora dolÅ¯.<br/>SvÄ›t - 100 kontinentÅ¯ (10x10)<br/>Kontinent - 400 sektorÅ¯ (20x20)<br/>Sektor - 25 polÃ­ (5x5)">',
				defaultLabel:' ',
				example:'55:12:20',
				enabled: false
			},
	...
	content.saveConfig = function()
	{
		...
		UI.InfoMessage('NastavenÃ­ uloÅ¾eno.',3000,'success');
	};
	...
	content.nameVillages = function()
	{
		...
		{
			UI.InfoMessage('NÃ¡zvy jsou pÅ™Ã­liÅ¡ dlouhÃ©! (max 32 znakÅ¯).',5000,'error');
		}				
		else
		{
		...
		content.innerHTML = '<b>&nbsp;NÃ¡hled nÃ¡zvu: </b>';
		...
		for(var i=0;i<options.length;i++)
		{
			...
			if(options[i].name == 'number_villages') //put the special numbering input in the label spot
			{
				...
				numberingInputTable.rows[0].cells[1].innerHTML = 'ÄŒÃ­slice';
		...
		modeForm.id = 'twcheese_name_mode_form';
			...
			modeForm.innerHTML += 'PÅ™epsat aktuÃ¡lnÃ­ nÃ¡zvy';
			...
			modeForm.innerHTML += 'PÅ™idat pred souÄasnÃ½ nÃ¡zev';			
			...
			modeForm.innerHTML += 'PÅ™idat za souÄasnÃ½ nÃ¡zev';
		...
		buttonDiv.style.padding = '10px';
			...
			saveButton.innerHTML = 'Nastavit jako vÃ½chozÃ­';
			buttonDiv.appendChild(saveButton);
			
			/*==== confirm button ====*/
			var confirmButton = document.createElement('a');
			confirmButton.className = 'btn-default btn-green';
			confirmButton.innerHTML = 'HromadnÄ› pÅ™ejmenovat';
...
{
    // Pool názvů – vytvoří se jen jednou na začátku spuštění
    if (typeof namePool === 'undefined') {
        namePool = [
            'Bramborová Lhota','U Tlustý Krávy','MegaChleba','TadyNicNení','HovnoVes','U Šikmýho Stromu',
            'Ves Pod Pivem','U Rozbitý Studny','Líná Prdel','Kraví Zátočina','Ovčí Údolí','Klobásová Ves',
            'Tři Opice','U Ubrečený Kozy','Pod Mostem','Zadní Prdelín','Mokrá Louka','Chlastov',
            'Bramborová Pevnost','Kuřecí Pahorek','Udivený Kámen','Funkční Ves?','Žumpadorf','Slepá Ulička',
            'Divná Díra','Zapomenutá Lhota','Šibeniční Kopec','Kokotín','U Ztracený Boty','Pičín Údolní',
            'Trollí Kout','Banánové Kralovství','Ves Smutných Brambor','Suchá Řeka','Pivní Přístav',
            'U Plačícího Buvola','Ohořelý Mlýn','Kloboučníkov','Turbodvůr','U Křivého Plotu',
            'Špagetová Ves','Zvrácená Bažina','Prokletá Bouda','Ves U Divných Zvuků','U Bohdalky','Smradov',
            'Králičí Hrad','Ovčín Naděje','Mastná Rokle','Čuňátkov','Zkažené Údolí','Náhodná Dera',
            'U Šílenýho Poníka','Špinavé Hřebeny','Opilecká Bašta','Uchlácholená Řepa','Králičí Štít',
            'Blbovice','U Stržené Štěpky','U Zmrzlých Nožiček','Hnijící Lúka','U Kyselé Okurky',
            'Zkažená Studna','U Slintající Kočky','Prdelní Útesy','Ves U Tichého Prdu','Smíchaná Lhota',
            'Připálená Ves','U Pichlavého Keře','Překvapený Vrch','U Zlomené Tyče','Smažený Důl',
            'Kopec Zbytečnosti','U Kňučícího Prasete','Ves U Oplácaného Stromu','Šílený Potok','Rozmaštěná Ves',
            'Šejdrem Dvůr','Blátivé Městečko','Podivná Skála','Ves Na Nabroušeném Hřebíku','Ošuntělé Údolí',
            'Nafoukaný Pahorek','Připosraný Jelen','Ves Na Jedno Oko','U Zběsilé Husy','Ramenová Rokle',
            'Vysmáté Paseky','U Uřvané Veverky','Ledabylec','Ves Na Kraji Kopru','Drbanice','Turbotonda',
            'U Fialového Krtka','Páchnoucí Seník','U Rohlíkové Zátoky','Brokolicov'
        ];
    }
    ...
    // Vyber náhodný index
    var randomIndex = Math.floor(Math.random() * namePool.length);

    // Vybraný název
    name = namePool[randomIndex];

    // Odstraníme, aby se v tomto spuštění už nepoužil
...
// --- KONEC NOVÉHO BLOKU ---
			...
			if (name.length <= 32) {
				...
				UI.InfoMessage('NÃ¡zev je pÅ™Ã­liÅ¡ dlouhÃ½ (max. 32 znakÅ¯).<br/>NovÃ½ nÃ¡zev nebude aplikovÃ¡n na vÅ¡echny vesnice.',5000,'error');
			}
	...
	if(nameSaveButtons.length > 0){
		window.setTimeout(
			function(){
				...
				if(nameSaveButtons.length > 0){
					UI.InfoMessage( ('PÅ™ejmenovanÃ© vesnice #'+savedCounter),5000,'success');
					saveVillageNames();
				}else{
					UI.InfoMessage('NÃ¡zvy vesnic byly ÃºspÄ›Å¡nÄ› zmÄ›nÄ›ny.',5000,'success');
				}
			},
			saveDelay
		);
...
/*==== main ====*/
	...
	if (game_data.screen == 'overview_villages' || canNameVillages) {
		...
		UI.InfoMessage('PouÅ¾ijte v nÃ¡hledu vesnic.',5000,'error');
	}
